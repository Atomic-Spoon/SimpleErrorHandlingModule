...
Function Invoke-ErrorHandling{
    <#
      ...
      .PARAMETER WriteToLogFile
        ...
        NOTE: If no logfile path is defined (using the -LogFilePath paraameter - see below) the module will write to to a auto-generated file in ~\Documents\
  
      .PARAMETER LogfilePath  
        
        [Optional] The path to a desired log file, used with the -WriteToLogFile option, if the log file path is not defined then the module will write to to a auto-generated file in ~\Documents\ 
      ...
      .EXAMPLE
        ...
        *** ENCOUNTERED RECOVERABLE ERROR ***
        ...
        LineNumber          : 3
        ScriptTextInLine    : Get-ADUser -justaFakeParam
      ...
      .EXAMPLE
        ...
        *** ENCOUNTERED RECOVERABLE ERROR ***
        ...
        ErrorMessage        : A parameter cannot be found that matches parameter name 'justaLoggingFakeParam'.
        ExceptionType       : ParameterBindingException
        ExceptionID         : NamedParameterNotFound
        LineNumber          : 3
        ScriptTextInLine    : Get-ADUser -justaLoggingFakeParam
    ...
    # Some formatting variables to save redundant code lines
    If ($FatalError.IsPresent)
    {
      $isItFatalDoc = "FATAL ERROR"
      $colour = "Red"
    } 
    ELSE 
    {
      $isItFatalDoc = "RECOVERABLE ERROR"
      $colour = "Yellow" 
    }
    <# Error items #>
    
    # Extract the error messgage from the last error in the PS Error Stack
    $errorExceptionMsg = $_.ErrorDetails.Message
    # If the objects .ErrorDetails.Message is $null grab the other spot where we can extract the error message from the object
    If (!$errorExceptionMsg) 
    {
    ...
    Write-Host -ForegroundColor $colour "`n*** ENCOUNTERED $isItFatalDoc ***"
    #Write-Host "`n$isItFatalDoc DETAILS:`n"
    <# Create a results hashtable to give some easy formatting #>
    $ErrorHashTable = [ordered]@{ErrorMessage=$errorExceptionMsg;
                        ExceptionType = $errorExceptionType;
                        ExceptionID=$errorExceptionId;
                        LineNumber=$errorInvocationLineNumber;
                        ScriptTextInLine=$($errorInvocationFullScriptLine.trim())}
    # Using [PSCustomOject] when pushing the hastable output to screen will take care of tabs and alignnment etc so we don't need to code it
    ...
    If ($writeToLogFile.IsPresent)
    {
      If ([string]::IsNullOrEmpty($LogFilePath))
      {
        ...
        $LogFilePath = "~\Documents\$autoFileName"
      ...
      <# Lets push the conents of $ErrorHashTable as string values to the log file#>
      ...
      <# Verbose PS cmdlet info for debugging if required #>
    ...
    If ($FatalError -eq $true ) 
    {
      ...
      If ($writeToLogFile.IsPresent)
      {
        <# The value for LogFilePath should have been sorted above #>
        Write-Logfile -logFilePath $LogFilePath "`n!!! HALTING SCRIPT  !!!"
      }
